...
위와 같이 함수가 아닌 조건문 또는 반복문의 경우 var로 선언한 변수는 지역 스코프로 인정하지 않는다.
즉, 함수가 아니면 모두 전역으로 판단하고 있는 것이다.
...
> 만약 동적 스코프로 작동한다면 위의 함수는 어떤 값을 출력할까?
> 동적 스코프(dynamic scope)란 함수가 호출되는 시점에 동적으로 상위 스코프를 결정하는 것을 의미한다.
> 그렇다면 각자 함수는 실행되는 시점에서의 변수 x를 찾을 것이다.
> foo 함수가 실행되고 내부 함수로 bar가 호출되었는데 이 때 bar 함수의 상위 스코프는 foo 함수 스코프가 되고 10이 출력된다.
> bar도 역시 실행되는 시점에 동적으로 전역 스코프가 상위 스코프로 결정되고 1이 출력된다.

함수를 어디서 정의했는지에 따라 함수의 상위 스코프가 결정되는 것을 렉시컬 스코프(lexical scope) 또는 정적 스코프(static scope)라 한다.
자바스크립트는 렉시컬 스코프를 따르므로 함수를 어디서 호출했는지가 아닌 함수를 어디서 정의했는지에 따라 상위 스코프를 결정한다.
함수가 호출된 위치는 상위 스코프 결정에 어떠한 영향도 주지 않는다.

<br>

위 예제에서 bar 함수는 전역에서 정의된 함수이다. 이 bar 함수는 전역 코드가 실행되기 전에 먼저 평가되어 함수 객체를 생성하고 이 객체는 자신이 정의된 스코프, 즉 전역 스코프를 기억한다.
